	...
	public static void main(String arg[]) throws IOException
	{
		...
		while(t-->0){
			...
			for(i=0;i<n;i++){
				...
				if(population[i]>Globalmax){
					Globalmax = population[i];
					Globalindex = i;
				}
			...
			//System.out.println("Gop"+Globalmax+"  Globalindex"+Globalindex);
			for(i=0;i<n;i++){
				if(Globalindex!=i && !map[i].contains(Globalindex)){
					pw.print((Globalindex+1)+" ");
				}else{
				//	System.out.println("Entering for i = "+i+" Values are : ");	
					temp_population[i]=0;
					...
					for(int j=0;j<n;j++){
						...
						temp_population[j] = population[j];
					}	
					pw.print(index+1+" ");	
				}
			}
